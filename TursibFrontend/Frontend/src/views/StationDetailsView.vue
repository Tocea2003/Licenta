<template>
  <div class="station-details-page">
    ...
    <div v-else-if="error" class="error-state">
      <p>❌ {{ error }}</p>
      <button @click="loadStationData" class="retry-btn">Reîncearcă</button>
    </div>

    <div v-else class="content">
      <!-- Live ETAs Section -->
      <div class="etas-section">
        <h2>🚌 Autobuze în Apropiere</h2>
        <div v-if="liveETAs.length === 0" class="empty-state">
          <p>Nu sunt autobuze programate în următoarele 60 de minute</p>
        </div>
        <div v-else class="etas-list">
          ...
          >
            ...
            <div class="countdown">
              ...
              <span class="label">{{ eta.countdown < 60 ? 'secunde' : 'minute' }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="routes-section">
        <h2>🔄 Trasee care Trec prin Stație</h2>
        <div v-if="routes.length === 0" class="empty-state">
          <p>Nu sunt trasee disponibile</p>
        </div>
...
const addToFavorites = () => {
  ...
  alert('✅ Stația a fost adăugată la favorite!')
}

const enableNotifications = () => {
  alert('🔔 Notificările pentru această stație vor fi activate în curând!')
}
